document.addEventListener("DOMContentLoaded", function () {
    ...
    function searchFilms(searchTerm) {
        ...
        let matchedFilms = films.filter(function (film) {
            return film.title.toLowerCase().includes(searchTerm.toLowerCase());
        });

        if (matchedFilms.length > 0) {
            matchedFilms.forEach(function (film) {
                let filmElement = document.createElement('p');
                filmElement.textContent = film.title + ' - Année de sortie : ' + film.year;
                resultsDiv.appendChild(filmElement);
            });
        } else {
            resultsDiv.textContent = "Aucun film trouvé";
        }
    }
});
